...
achieve **0.51-0.66x** of `lru_cache`'s single-threaded throughput while
providing native thread safety that delivers **1.4-1.5x** higher throughput
...
- **14-20M ops/s** single-threaded, **18M ops/s** under concurrent load, **18-24x** faster than `cachetools`
...
```bash
pip install -i https://test.pypi.org/simple/ fast_cache
```
...
| Metric | fast_cache | cachetools | lru_cache |
|---|---|---|---|
| Single-threaded | 14-20M ops/s | 0.6-1.2M ops/s | 21-40M ops/s |
| Multi-threaded (8T) | 17M ops/s | 770K ops/s (with Lock) | 12M ops/s (with Lock) |
| Thread-safe | Yes (RwLock) | No (manual Lock) | No |
| Async support | Yes | No | No |
| Cross-process (shared) | Yes (mmap) | No | No |
| TTL support | Yes | Yes | No |
| Eviction strategies | LRU, MRU, FIFO, LFU | LRU, LFU, FIFO, RR | LRU only |
| Implementation | Rust (PyO3) | Pure Python | C (CPython) |

Under concurrent load, `fast_cache` delivers **1.4-1.5x** higher throughput than `lru_cache + Lock` and **18-24x** higher than `cachetools`. See [full benchmarks](docs/performance.md) for details.
